...
//todo Box is used to give the dynamic error a fixed size
fn read_and_validate(b: &mut dyn io::BufRead) -> Result<PositiveNonzeroInteger, Box<dyn error::Error>> {
    ...
    let num: i64 = line.trim().parse()?; //parse int error
    let answer = PositiveNonzeroInteger::new(num)?; //creation error
    Ok(answer) //todo when we define the return type as a Result, the positive answer must always be Ok()
}
